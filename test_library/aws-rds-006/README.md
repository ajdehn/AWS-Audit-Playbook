...
2. For each in-scope region, saved the list of RDS instances: [rds/[region_name]/db_instances.json](/evidence_library/rds/us-east-1/db_instances.json).
3. For each RDS instance, reviewed the `TagList` to determine if the required tags exist and contain non-empty values.
  * By default, the python library requires the following tags: "Owner", "Description", "Classification".
  * If you want to change the default required tags, please update the AWSConfig values.
